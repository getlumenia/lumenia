...
- ✅ ~~`apps/sponsor` HTTP service~~ → **live as a Cloudflare Worker** (deployed twice: testnet + the mainnet pilot) with the anti-drain gate, fee cap, canary caps and per-IP/per-account rate limit (§10). Still open from the old sub-list: **provisioning** the KMS key (the signer is wired behind `KMS_KEY_ID` + 13/13 offline, but both deployments still run the env hot-key — §11), and putting a **timelock** in front of the mainnet contract owner, which has been a **2-of-3 multisig** since 2026-09-18 (all three keys founder-held today, one still to move off the laptop).
