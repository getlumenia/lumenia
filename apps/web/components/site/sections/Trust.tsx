...
const FAQ = [
  ...
  { q: "I don’t have a wallet. Where is my money, and my key? Is it safe?", a: "You don’t download a wallet, because Lumenia never holds your money for you. The key to it lives on your own phone, locked, and there is no copy of it on our servers. We can’t see it and we can’t move your money. That is the whole point: even if we were hacked or shut down tomorrow, nobody could take your money from us, because it was never with us. Two things keep it safe on your side. Lock it with a password (or your face) so a stranger holding your phone can’t spend it, and set up a backup so a lost phone isn’t a lost balance." },
  { q: "If I lose my phone, can I get my money back?", a: "Yes, if you set up a backup first, so do it right after you claim. With your email (and your face, if your phone offers it) you can bring your money back on a new phone. The backup is an encrypted copy of your key: we store the locked box, never the key itself, so even then we can’t see your money. If you skip the backup and then lose the phone, reaching that money becomes very hard, which is exactly why we nudge you to back up the moment it arrives." },
  { q: "What if I forget my password?", a: "There is no password reset, and we would rather say that plainly than pretend. Your password is the master key to your money, and Face ID is only a shortcut on top of it, not a replacement. Nobody, us included, can unlock it for you, because we built it so we never can. That is what keeps it yours alone. So set up the backup (your email, and your face if you can) as your safety net, and pick a password you won’t lose. One thing to keep separate: forgetting the password on a link you SEND loses nothing, that money returns to you after 7 days. This is about the password on your own account." },
  { q: "How do I know a Lumenia link or page is real, and not a scam?", a: "One rule keeps you safe: Lumenia lives at one address, getlumenia.com. We will never send you a different-looking link, and we will never ask for your password on any other site. If a page does, it isn’t us. A real money link opens your money straight away; it never asks you to “verify” by typing your password into a strange site first. When in doubt, type getlumenia.com yourself instead of tapping a link someone forwarded." },
  { q: "Can my dollars be frozen?", a: "Honestly, yes, but not by us, and it is rare. The dollars are a regulated digital dollar (USDC, issued by Circle), and like any regulated dollar, a balance flagged for sanctions or fraud can be frozen by the issuer. We can’t freeze your money and we can’t unfreeze it: we don’t control the dollar itself, we only move it. For everyday transfers to family this practically never comes up. We mention it because we would rather you hear it from us than be surprised." },
